...
int main() {
	IplImage* lena = cvLoadImage("lena.jpg", CV_LOAD_IMAGE_GRAYSCALE); // 기존 이미지 로드
	IplImage* heart = cvLoadImage("heart.jpg", CV_LOAD_IMAGE_GRAYSCALE); // 합성할 이미지 로드
	IplImage* Rock = cvLoadImage("Rock.jpg", CV_LOAD_IMAGE_GRAYSCALE); // 합성할 이미지 로드
	IplImage* scaleDownImage = cvCreateImage(cvSize(lena->width, lena->height), lena->depth, lena->nChannels); // Scale down 이미지 생성
	IplImage* rotationImage = cvCreateImage(cvSize(lena->width, lena->height), lena->depth, lena->nChannels);  // 회전 이미지 생성
...
IplImage *ScaleDown(IplImage *sclaeDownImage) {
	...
	Mask1 = new double[SUB_SAMPLING_RATE * SUB_SAMPLING_RATE]; // 마스크 크기
	Mask2 = new double[SUB_SAMPLING_RATE * SUB_SAMPLING_RATE]; // 마스크 크기
	...
	for (i = 0; i < sclaeDownImage->height; i++) { // 인풋이미지 temp에 옮김
		...
		for (j = 0; j < sclaeDownImage->width; j = j + SUB_SAMPLING_RATE) {
			for (n = 0; n < SUB_SAMPLING_RATE; n++) {
				for (m = 0; m < SUB_SAMPLING_RATE; m++) {
					...
					Mask1[n*SUB_SAMPLING_RATE + m] = tempScalar.val[1]; // 마스크 범위의 화소값들 배열 만듦
					Mask2[n*SUB_SAMPLING_RATE + m] = tempScalar.val[2]; // 마스크 범위의 화소값들 배열 만듦
				}
			}
	...
	for (i = 0; i < scaleDownImage->height; i++) {
		for (j = 0; j < scaleDownImage->width; j++) {
			...
			if (pixelValue.val[0] != 255) {
				temp = cvGet2D(scaleDownImage, i, j); // 그외는 원본 값
				cvSet2D(mixImage, i, j, temp);
			}
			else {
				temp = cvGet2D(mainImage, i, j); // 합성할 이미지 보다 어두우면 그 색을 설정
				cvSet2D(mixImage, i, j, temp);
			}
		}
	}
