...
IplImage* ConvolutionProcess(IplImage* inputImage, double Mark[3][3]);

int main() {
	IplImage* inputImage = cvLoadImage("lena.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	IplImage* ResultImage = NULL;

	double EmboMask[3][3] = { { -1.,0.,0. },{ 0.,0.,0. },{ 0.,0.,1. } }; // 엠보싱 마스크
...
IplImage* ConvolutionProcess(IplImage* inputImage, double Mask[3][3]) {
	...
	for (i = 0; i < inputImage->height; i++) { // 컨벌루션 연산
		for (j = 0; j < inputImage->width; j++) {
			...
			//cvSet2D(outputImage, i, j, cvScalar(Sum)); // 결과값을 아웃풋 이미지에 넣음
			cvSet2D(outputImage, i, j, cvScalar(Sum + 128)); // 엠보싱 처리 입력 영상을 양각 형태로 보이게 하는 기술
			Sum = 0.0;
		}
	}
